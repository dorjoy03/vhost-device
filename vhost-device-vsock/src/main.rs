...
use crate::vhu_vsock::{CidMap, VhostUserVsockBackend, VsockConfig, ProxyType, VsockProxyInfo};
...
struct VsockParam {
    ...
    #[arg(long, conflicts_with="forward_cid", conflicts_with = "config", conflicts_with = "vm")]
    uds_path: Option<String>,

    /// The vsock CID to forward connections from guest
    #[clap(long, conflicts_with = "uds_path", conflicts_with="config", conflicts_with = "vm")]
    forward_cid: Option<u32>,
    
     /// The vsock ports to forward connections from host
    #[clap(long, conflicts_with = "uds_path", conflicts_with="config", conflicts_with = "vm")]
    vsock_listen: Option<String>,
...
fn parse_vm_params(s: &str) -> Result<VsockConfig, VmArgsParseError> {
    ...
    let mut vsock_listen: Option<Vec<u32>> = None;
    ...
    for arg in s.trim().split(',') {
        ...
        match key {
            ...
            "vsock_listen" | "vsock-listen" => {
                vsock_listen = Some(val.split('+').map(|s| s.parse().unwrap()).collect())
            }
    ...
    let listen_ports: Vec<u32> = match vsock_listen {
        None => Vec::new(),
        Some(ports) => ports,
    };

    let proxy_info = match (uds_path, forward_cid) {
        (Some(path), None) => ProxyType::UnixDomainSocket(path),
        (None, Some(cid)) => ProxyType::Vsock(VsockProxyInfo {forward_cid: cid, listen_ports: listen_ports}),
        (None, None) => return Err(VmArgsParseError::RequiredKeyNotFound("uds-path or forward-cid".to_string())),
        _ => return Err(VmArgsParseError::RequiredKeyNotFound("Only one of uds-path or forward-cid can be provided".to_string())),
    };
                ...
                figment.extract::<HashMap<String, Vec<ConfigFileVsockParam>>>()
            {
                ...
                if !vms_param.is_empty() {
                    ...
                    for p in vms_param.drain(..) {
                        let listen_ports: Vec<u32> = match p.vsock_listen {
                            None => Vec::new(),
                            Some(ports) => ports.split('+').map(|s| s.parse().unwrap()).collect(),
                        };
                        let proxy_info = match (p.uds_path, p.forward_cid) {
                            (Some(path), None) => ProxyType::UnixDomainSocket(path.trim().to_string()),
                            (None, Some(cid)) => ProxyType::Vsock(VsockProxyInfo {forward_cid: cid, listen_ports: listen_ports}),
...
mod tests {
        ...
        fn from_args_vsock(
            ...
            VsockArgs {
                param: Some(VsockParam {
                    ...
                    vsock_listen: Some(vsock_listen.to_string()),
                    tx_buffer_size,
                    queue_size,
                    groups: groups.to_string(),
                }),
    ...
    fn test_vsock_config_setup_unix() {
        ...
        assert_eq!(config.get_proxy_info(), ProxyType::UnixDomainSocket(uds_path));
    ...
    fn test_vsock_config_setup_vsock() {
        ...
        let socket_path = test_dir.path().join("vhost4.socket").display().to_string();
        let args = VsockArgs::from_args_vsock(3, &socket_path, 1, "1234+4321", 64 * 1024, 1024, "group1");
        ...
        assert_eq!(config.get_guest_cid(), 3);
        assert_eq!(config.get_socket_path(), socket_path);
        assert_eq!(config.get_proxy_info(), ProxyType::Vsock(VsockProxyInfo {forward_cid: 1, listen_ports: vec![1234, 4321]}));
        assert_eq!(config.get_tx_buffer_size(), 64 * 1024);
        assert_eq!(config.get_queue_size(), 1024);
        assert_eq!(config.get_groups(), vec!["group1".to_string()]);
    ...
    fn test_vsock_config_setup_from_vm_args() {
        ...
        let params = format!(
            "--vm socket={vhost3_socket},uds_path={vm3_vsock} \
             ...
             --vm guest-cid=6,socket={vhost6_socket},forward-cid=1,vsock-listen=1234+4321,queue-size=2048",
        ...
        assert_eq!(config.get_proxy_info(), ProxyType::UnixDomainSocket(uds_paths[0].display().to_string()));
        ...
        assert_eq!(config.get_proxy_info(), ProxyType::UnixDomainSocket(uds_paths[1].display().to_string()));
        assert_eq!(config.get_tx_buffer_size(), 65536);
        assert_eq!(config.get_queue_size(), 1024);
        assert_eq!(config.get_groups(), vec!["group1".to_string()]);
        ...
        assert_eq!(config.get_proxy_info(), ProxyType::UnixDomainSocket(uds_paths[2].display().to_string()));
    ...
    - guest_cid: 5
      ...
      tx_buffer_size: 32768",
        ...
        let config = &configs[0];
        assert_eq!(config.get_guest_cid(), 4);
        assert_eq!(config.get_socket_path(), socket_path_unix.display().to_string());
        assert_eq!(config.get_proxy_info(), ProxyType::UnixDomainSocket(uds_path.display().to_string()));
        assert_eq!(config.get_tx_buffer_size(), 32768);
        assert_eq!(config.get_queue_size(), 256);
        assert_eq!(
            config.get_groups(),
        ...
        assert_eq!(config.get_guest_cid(), 5);
        assert_eq!(config.get_socket_path(), socket_path_vsock.display().to_string());
        assert_eq!(config.get_proxy_info(), ProxyType::Vsock(VsockProxyInfo {forward_cid: 1, listen_ports: vec![1234, 4321]}));
        assert_eq!(config.get_tx_buffer_size(), 32768);
        assert_eq!(config.get_queue_size(), 1024);
        assert_eq!(
    ...
    fn test_vsock_server_vsock() {
        ...
        let config = VsockConfig::new(
            CID,
            vhost_socket_path,
            ProxyType::Vsock(VsockProxyInfo {forward_cid: 1, listen_ports: vec![9000]}),
            CONN_TX_BUF_SIZE,
            QUEUE_SIZE,
            vec![DEFAULT_GROUP_NAME.to_string()],
        );
        ...
        let configs = [
            VsockConfig::new(
                ...
                test_dir
                    ...
                    .display()
                    .to_string(),
                ProxyType::UnixDomainSocket(test_dir
                    .path()
    ...
    fn test_main_structs() {
        ...
        assert_eq!(format!("{args:?}"), "VsockArgs { param: Some(VsockParam { guest_cid: 0, socket: \"\", uds_path: Some(\"\"), forward_cid: None, vsock_listen: None, tx_buffer_size: 0, queue_size: 0, groups: \"\" }), vm: None, config: None }");

        let param = args.param.unwrap().clone();
        assert_eq!(format!("{param:?}"), "VsockParam { guest_cid: 0, socket: \"\", uds_path: Some(\"\"), forward_cid: None, vsock_listen: None, tx_buffer_size: 0, queue_size: 0, groups: \"\" }");

        let args = VsockArgs::from_args_vsock(0, "", 1, "", 0, 0, "");
        assert_eq!(format!("{args:?}"), "VsockArgs { param: Some(VsockParam { guest_cid: 0, socket: \"\", uds_path: None, forward_cid: Some(1), vsock_listen: Some(\"\"), tx_buffer_size: 0, queue_size: 0, groups: \"\" }), vm: None, config: None }");

        let param = args.param.unwrap().clone();
        assert_eq!(format!("{param:?}"), "VsockParam { guest_cid: 0, socket: \"\", uds_path: None, forward_cid: Some(1), vsock_listen: Some(\"\"), tx_buffer_size: 0, queue_size: 0, groups: \"\" }");
        ...
        .clone();
        assert_eq!(format!("{config:?}"), "ConfigFileVsockParam { guest_cid: None, socket: \"\", uds_path: Some(\"\"), forward_cid: None, vsock_listen: None, tx_buffer_size: None, queue_size: None, groups: None }");
        ...
        let config = ConfigFileVsockParam {
            ...
            uds_path: None,
            forward_cid: Some(1),
            vsock_listen: Some(String::new()),
            tx_buffer_size: None,
            queue_size: None,
            groups: None,
        }
        .clone();
        assert_eq!(format!("{config:?}"), "ConfigFileVsockParam { guest_cid: None, socket: \"\", uds_path: None, forward_cid: Some(1), vsock_listen: Some(\"\"), tx_buffer_size: None, queue_size: None, groups: None }");
    }
}
